...
flags.DEFINE_bool(
    "do_lower_case", True,		# True: 전부 소문자로, False: 대, 소문자 구분
    "Whether to lower case the input text. Should be True for uncased "
    "models and False for cased models.")

flags.DEFINE_integer("max_seq_length", 128, "Maximum sequence length.")	# sequence length를 제한

flags.DEFINE_integer("max_predictions_per_seq", 20,	# [MASK] token의 최대 개수
...
def create_training_instances(input_files, tokenizer, max_seq_length,
  ...
  print('!!!!!!input_files name = ',str(input_files))
  for input_file in input_files:
    ...
    #with tf.gfile.GFile(input_file, "r") as reader:
      while True:
        ...
        if tokens:
          all_documents[-1].append(tokens)	# 문서 list에 추가
  ...
  rng.shuffle(all_documents)			# 문서 단위로 shuffle
  tf.logging.info('make vocab dict')
  vocab_words = list(tokenizer.vocab.keys())
  instances = []
  tf.logging.info('make tokens')
#   print("all_documents : {}".format(all_documents))
#   print("dupe_factor : {}".format(dupe_factor))
  for _ in range(dupe_factor):
    for document_index in range(len(all_documents)):	# 문서마다 반복
...
def create_instances_from_document(
  ...
  max_num_tokens = max_seq_length - 3		# 512 - 3
						# input token 개수인 512개에서 필수로 포함되는 3가지 token을 제거
  ...
  if rng.random() < short_seq_prob:
    target_seq_length = rng.randint(2, max_num_tokens)				# 1. 2-509 만큼 random 선택 
										# 509개의 sequence length중, 학습을 위해 채울 sequence length를 dynamic하게 결정
  ...
  while i < len(document):							# 2. 해당 document의 문장들을 가져옴
    ...
    if i == len(document) - 1 or current_length >= target_seq_length:		# 5. 목표치 sequence length에 도달했다면
      if current_chunk:
        # `a_end` is how many segments from `current_chunk` go into the `A`
        # (first) sentence.
        a_end = 1
        if len(current_chunk) >= 2:
          a_end = rng.randint(1, len(current_chunk) - 1)			# 6. 문장 A의 끝을 random하게 선택
        ...
        if len(current_chunk) == 1 or rng.random() < 0.5:			# 8. 50%의 확률로 IsNext or NotNext
          is_random_next = True
          target_b_length = target_seq_length - len(tokens_a)			# 9. NotNext라면 ??
          ...
          random_document = all_documents[random_document_index]		# 10. Random으로 다음 문장 선택한 후 붙여줌
...
#     tf.logging.info("  %s", output_file)
  ...
  write_instance_to_example_files(instances, tokenizer, FLAGS.max_seq_length,	# output 파일에 write를 수행하는 부분
